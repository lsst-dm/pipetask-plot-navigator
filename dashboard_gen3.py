...
repo_select = pn.widgets.Select(name='Repository', options=[p for p in Path(root_entry.value).glob('*') 
...
plot_select = pn.widgets.MultiSelect(
    name="Plots",
    options=[],
)
...
def update_plot_names(event):    
    ...
    for tract in tract_select.value:
        refs = [ref for ref in registry.queryDatasets(pattern, collections=collection_select.value, findFirst=True,
