...
def find_skymaps(registry, collection):
    return list(set(x.dataId["skymap"] for x in registry.queryDatasets("skyMap", collections=collection)))
...
class PlotExplorer(param.Parameterized):
    ...
    skymap = param.ObjectSelector(objects=[])
    ...
    def _update_tract_select(self):
        ...
        if registry is not None:
            try:
                ...
                self.param.skymap.objects = find_skymaps(registry, self.collection1)
                self.skymap = self.param.skymap.objects[0]
            except:
                raise
        ...
        for tract in self.tract:
            refs = [
                ...
                for ref in registry.queryDatasets(
                    ...
                    findFirst=True,
                    dataId={"tract": tract, "skymap": self.skymap},
                )
    ...
    def plots(self):
        if self.plot_select is not None:
            plot_list = [
                pn.pane.PNG(self.plot_paths[name], width=self.plot_width) for name in self.plot_select
            ]
            plot_gbox = pn.GridBox(*plot_list, ncols=self.ncols)

            if self.plot_paths2:
                plot_list2 = [
                    pn.pane.PNG(self.plot_paths2[name], width=self.plot_width) for name in self.plot_select
                ]
    ...
    def panel(self):
        ...
        repo_collection_tabs = pn.Tabs(
            ("Collection 1", pn.Column(self.param.repo1, self.param.load1, self.param.collection1)),
            ("Collection 2", pn.Column(self.param.repo2, self.param.load2, self.param.collection2)),
        )

        gspec[0:3, 0:2] = repo_collection_tabs

        gspec[3, 0:2] = self.param.skymap
        gspec[4, 0:2] = self.param.tract
        gspec[5, 0:2] = self.param.name_filter
        gspec[6:12, 0:2] = self.param.plot_select

        gspec[0, 2:4] = self.debug
        gspec[1, 2] = pn.Param(
            self.param.plot_width,
            widgets={
                "plot_width": {"widget_type": pn.widgets.IntInput, "start": 300, "end": 1200, "step": 50}
            },
        )
